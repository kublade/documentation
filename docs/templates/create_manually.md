...
Here's an example of a complete template structure:
  ...
  template:
    ...
    spec:
      ...
      - name: {{ $data['app_name'] }}
        ...
        - name: metrics
          containerPort: {{ $data['metrics_port'] ?? 9090 }}
        @if($limits['enabled'])
        resources:
          limits:
            cpu: {{ $limits['cpu'] }}
...
- [Import from Helm chart](create_import.md): Learn how to import existing Helm charts
- [Blade templating](use_blade.md): Detailed guide on using the Blade templating engine
- [Port reservations](use_ports.md): How to manage port claims and reservations
